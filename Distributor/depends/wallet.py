##Edited from https://github.com/P5vc/Zecwallet-Python/blob/main/zecwallet/wallet.py - Original
##Apart of pip's zecwallet library
##Need to allow for editing server when initializing as well as allow for multiple wallets in the
##case that multiple elections can take place.
##Converted to work with Zingolib -- Regtest
...
class Wallet():
	def __init__(self , WALLET_EXEC_FULL_PATH , SERVER ,WALLET_NAME ='Wallet1' ,WALLET_DECRYPTION_KEY = ''):
		self._WALLET_DECRYPTION_KEY = WALLET_DECRYPTION_KEY
		self._sp = Popen([WALLET_EXEC_FULL_PATH, '--server', SERVER,'--data-dir','./data/'+WALLET_NAME] , stdout = PIPE , stdin = PIPE)
		self._readOutput()
		self._sendCommand('\r')
	...
	def addressBalance(self , targetAddress , fullResult = False):
		'''
		...
		if ('uregtest1' in targetAddress):
			return balances['orchard_balance']
		elif ('zregtestsapling1' in targetAddress):
			return balances['sapling_balance']
		elif ('t' == targetAddress[0]):
			return balances['transparent_balance']
		...
		# 	raise RuntimeError('The provided address was not found or is invalid.')
		else:
	...
	def _fetchResult(self):
		...
		logging.info("RawOutput LEN: " + str(len(rawOutput)))
		loadableOutput = ''
